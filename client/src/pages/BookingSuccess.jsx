...
import { useGetBookingQuery, useConfirmBookingMutation } from '../store/bookingApi'
...
const BookingSuccess = () => {
  ...
  const { data: bookingData, isLoading, error, refetch } = useGetBookingQuery(bookingId, {
    skip: !bookingId || !user || (!['pending', 'success'].includes(paymentStatus)),
    pollingInterval: paymentStatus === 'pending' && pollCount < maxPolls && user ? 2000 : 0, // Poll every 2 seconds if pending
  })
        ...
        if (statusCode === '2') {
          ...
          setTimeout(() => {
            if (user && token) {
              ...
              console.log('User or token not available, falling back to pending status')
              setPaymentStatus('pending')
              toast.error('Authentication required. Please confirm manually below.')
            }
          }, 2000)
        }
      } else {
  ...
  useEffect(() => {
    if (pollCount >= maxPolls && paymentStatus === 'pending') {
      setPaymentStatus('failed')
      toast.error('Payment verification timed out. Please check your dashboard or contact support.')
    }
  ...
  const isStudioOwner = booking && user && user.role === 'studio' && 
    booking.studio?.user?._id === user._id

  if (isLoading || paymentStatus === 'verifying' || paymentStatus === 'processing') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-gray-950 to-black flex items-center justify-center">
        ...
        >
          ...
          <p className="text-gray-400">
            {paymentStatus === 'processing' ? 'Confirming your booking...' : 'Verifying your payment...'}
          </p>
        </motion.div>
      </div>
    )
          ...
          >
            <Card className="p-8 bg-gray-800/50 border-gray-700/50 backdrop-blur-sm">
              ...
              <p className="text-gray-400 mb-6">
                Your session has expired. Please login again to view your booking status.
              </p>
              ...
              {isStudioOwner && (
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4 mb-6">
                  <p className="text-blue-300 text-sm">
                    <strong>Studio Owner:</strong> If you've received payment confirmation, 
                    you can manually confirm this booking below.
                  </p>
                </div>
              )}
                  ...
                  <>
                    <Button 
                      onClick={() => {
                        ...
                        // Also display in UI
                        toast.success(`Debug: Status=${paymentStatus}, Polls=${pollCount}/${maxPolls}, Error=${error?.status || 'none'}`)
                      }}
                      variant="outline"
                    ...
                    {bookingId && (
                      <Button 
                        onClick={async () => {
                          try {
                            const response = await fetch('/api/webhooks/test-webhook', {
                              method: 'POST',
                              headers: {
                                'Content-Type': 'application/json',
                              },
